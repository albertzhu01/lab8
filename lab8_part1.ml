...
module MakeInterval (Endpoint : ORDERED_TYPE) =
  struct
    ...
    let contains (intvl : interval) (x : Endpoint.t) : bool =
      match intvl with
      | Interval (a, b) -> compare a x <= 0 && compare b x >= 0
      | Empty -> false
    ...
    let intersect (intvl1 : interval) (intvl2 : interval) : interval =
      ...
      if contains intvl2 a && contains intvl2 b then Interval (a, b)
      else if contains intvl2 a && not(contains intvl2 b) then Interval (a, d)
      else if contains intvl1 c && contains intvl1 d then Interval (c, d)
      else if contains intvl1 c && not(contains intvl1 d) then Interval (c, b)
      else Empty
      |Empty, Interval _
      |Interval _, Empty
      |Empty, Empty -> Empty
...
let intvl2 =
  let open IntInterval in
    create 3 4;;
let intvl1_intersect_intvl2 =
  let open IntInterval in
    intersect intvl1 intvl2
;;
